...
export default function Newsletter({ textButton = false, lang }: TNewsletterProps) {
    const dict = getDictionary(lang).newsletter.form.emailInput;
    const {
        register,
        handleSubmit,
        formState: { errors },
            ...
            <div className="input-group">
                <input
                    ...
                    {...register("email", {
                        required: {
                            value: true,
                            message: dict.requiredError,
                        },
                        pattern: {
                            value: /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/,
                            message: dict.invalidError,
                        },
                    })}
                />
